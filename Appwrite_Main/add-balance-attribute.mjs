import { Client, Databases } from "node-appwrite";
const client = new Client()
  .setEndpoint("https://sgp.cloud.appwrite.io/v1")
  .setProject("69fb7f200039526c5d2e")
  .setKey("standard_");
const db = new Databases(client);
try {
  await db.createFloatAttribute("zenvyshop", "profiles", "balance", false, 0);
  console.log("✓ balance attribute added to profiles");
} catch(e) {
  if (e.code === 409) console.log("- balance already exists");
  else throw e;
}
